...
import com.sport.score.config.KafkaProperties;
...
class ScoreKafkaResourceIT {
    ...
    void setup() {
        ...
        ScoreKafkaResource kafkaResource = new ScoreKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/score-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/score-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
